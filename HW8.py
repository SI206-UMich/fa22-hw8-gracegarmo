...
def get_restaurant_data(db_filename):
    ...
    cur.execute('SELECT name, category, building, rating FROM restaurants JOIN categories on restaurants.category_id = categories.id JOIN buildings on restaurants.category_id = buildings.id')
    conn.commit()

    res = []

    for elt in cur:
        res.append({'name': elt[0], 'category': elt[1], 'building': elt[2], 'rating': elt[3]})
...
def barchart_restaurant_categories(db_filename):
    ...
    cur.execute('SELECT category, COUNT(*) AS occurrences FROM restaurants JOIN categories on restaurants.category_id = categories.id GROUP BY category')
    conn.commit()
    ...
    for i in range(len(res_sorted)):
        x.append(res_sorted[i][0])
        y.append(res_sorted[i][1])
